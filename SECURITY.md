...
AxiomDB stores data in a local SQLite file. Keep in mind:

- **The `.db` file is not encrypted.** Do not store secrets (API keys, passwords) in the cache without application-level encryption.
- **SQLite files are local.** AxiomDB is designed for single-server deployments. Do not expose the database file over a network share.
